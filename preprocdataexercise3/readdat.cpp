...
//				cout << line << '\n';
	...
	//Create the file Bpair.dat and bonddo.dat
	ofstream filebpairs ("Bpair.dat");
	if (filebpairs.is_open()){
		for (int i=0; i<Bpair.size(); ++i){
			filebpairs << Bpair[i] << "\n";
		}
		filebpairs.close();
	} else {
		cout << "Unable to open file: Bpair.dat" << endl;
	}

	ofstream filebdo ("bonddo.dat");
	if (filebdo.is_open()){
		...
		cout << "Unable to open file: bonddo.dat" << endl;
	}
/*
	for (int i=0; i<Bpair.size(); ++i) {
		cout << " " << Bpair[i] << endl;
...
//				cout << line2 << '\n';
	...
	//Create the file Atrip.dat and thetao.dat
	ofstream fileatrip ("Atrip.dat");
	if (fileatrip.is_open()){
		for (int i=0; i<Atrip.size(); ++i){
			fileatrip << Atrip[i] << "\n";
		}
		fileatrip.close();
	} else {
		cout << "Unable to open file: Atrip.dat" << endl;
	}

	ofstream fileathetao ("thetao.dat");
	if (fileathetao.is_open()){
		for (int i=0; i<thetao.size(); ++i){
			fileathetao << thetao[i] << "\n";
		}
		fileathetao.close();
	} else {
		cout << "Unable to open file: thetao.dat" << endl;
	}
/*
	for (int i=0; i<Atrip.size(); ++i) {
		cout << " " << Atrip[i] << endl;
...
//				cout << line3 << '\n';
	...
	//Create the file Tquad.dat and phio.dat
	ofstream filetquad ("Tquad.dat");
	if (filetquad.is_open()){
		for (int i=0; i<Tquad.size(); ++i){
			filetquad << Tquad[i] << "\n";
		}
		filetquad.close();
	} else {
		cout << "Unable to open file: Tquad.dat" << endl;
	}

	ofstream filetphio ("phio.dat");
	if (filetphio.is_open()){
		for (int i=0; i<phio.size(); ++i){
			filetphio << phio[i] << "\n";
		}
		filetphio.close();
	} else {
		cout << "Unable to open file: phio.dat" << endl;
	}
...
*/
	...
	if (file4.is_open())
	{
		...
		{ 
//			cout << "line4 size=" << line4.size() << "\n";

			if (line4.size() > 139) break;
...
//				cout << line4 << '\n';
	...
	//Create the file Ppairs.dat and pairsdo.dat
	ofstream fileppairs ("Ppairs.dat");
	if (fileppairs.is_open()){
		for (int i=0; i<Ppairs.size(); ++i){
			fileppairs << Ppairs[i] << "\n";
		}
		fileppairs.close();
	} else {
		cout << "Unable to open file: Ppairs.dat :( " << endl;
	}

	ofstream filepdo ("pairsdo.dat");
	if (filepdo.is_open()){
		...
		cout << "Unable to open file: pairsdo.dat" << endl;
	}
/*
	for (int i=0; i<Ppairs.size(); ++i) {
		cout << " " << Ppairs[i] << endl;
